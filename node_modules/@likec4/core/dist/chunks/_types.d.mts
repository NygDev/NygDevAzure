...
import { Ai as DeploymentFqn, At as ElementSpecification$1, Bn as ModelFqnExpr, Bt as DeploymentRelationship, Ci as _aux_d_exports, Dt as LikeC4ProjectStylesConfig, E as ParsedView, Ei as setProject, Er as RelationshipArrowType, Fr as ThemeColor, Gr as Any, Hn as Expression, It as DeploymentElement, J as ParsedDeploymentView, Kt as ModelRelation, Nr as SpacingSize, Nt as TagSpecification, Pi as Fqn, Pr as TextSize, Sn as AutoLayoutDirection, Vn as ModelRelationExpr, Wt as Element$1, Yr as Aux, an as ParsedDynamicView, er as Participant, ga as KeysOf, gn as ParsedElementView, jr as ShapeSize, jt as RelationshipSpecification, kr as RelationshipLineType, on as Step, pi as SpecAux, pr as BorderStyle, x as ParsedLikeC4ModelData, xn as AnyViewRuleStyle, xr as IconPosition, ya as NonEmptyArray, yr as ElementShape, zi as MarkdownOrString } from "./index.mjs";
...
//#endregion
//#region src/builder/Builder.deploymentModel.d.ts
 ...
 * ```
 */
<T extends AnyTypes, Source extends string & T['DeploymentFqn'], Target extends string & T['DeploymentFqn']>(source: Source, target: Target, titleOrProps?: string | Props) => (builder: DeploymentModelBuilder<T>) => DeploymentModelBuilder<T>;
...
//#endregion
//#region src/builder/Builder.element.d.ts
...
type RelationshipHelper<Props = unknown> =
/**
 * @example
 * ```ts
 * rel('a', 'b')
 ...
 * ```
 */
<T extends AnyTypes, From extends string & T['Fqn'], To extends string & T['Fqn']>(from: From, to: To, titleOrProps?: string | Props) => (builder: ModelBuilder<T>) => ModelBuilder<T>;
type NestedRelationshipHelper<Props = unknown> =
/**
 * @example
 * ```ts
 * el('a').with(
 ...
 * ```
 */
<T extends AnyTypesNested, To extends string & T['Fqn']>(to: To, titleOrProps?: string | Props) => (builder: ModelBuilder<T>) => ModelBuilder<T>;
...
declare function $expr<T extends AnyTypes>(expr: ViewPredicate.Expression<T> | Expression): Expression<Types.ToAux<T>>;
//#endregion
//#region src/builder/Builder.views.d.ts
interface ViewsBuilder<T extends AnyTypes> extends Builder<T> {
  __addView(view: ParsedView): Builder<T>;
}
...
//#endregion
//#region src/builder/Builder.d.ts
...
type ValidId<Id> = Id extends `${number}${string}` ? Invalid<'Id must not start with number'> : IsLiteral<Id> extends true ? Id : Invalid<'Id must be a literal'>;
/**
